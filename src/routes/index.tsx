  ...
  return (
    <AppLayout title="Dashboard" subtitle="Real-time overview of segregation activity">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <Kpi icon={<Trash2 className="h-5 w-5" />} label="Total Waste" value={`${kpi.total} kg`} tint="#3B82F6" />
        <Kpi icon={<Cog className="h-5 w-5" />} label="Metal Waste" value={`${kpi.metal} kg`} tint="#00E5FF" />
        <Kpi icon={<Droplets className="h-5 w-5" />} label="Wet Waste" value={`${kpi.wet} kg`} tint="#22C55E" />
        <Kpi icon={<Wind className="h-5 w-5" />} label="Dry Waste" value={`${kpi.dry} kg`} tint="#F59E0B" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mt-6">
        <Panel title="Waste Distribution">
          <div className="h-72">
            <ResponsiveContainer>
              <PieChart>
                <Pie data={distribution} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={70} outerRadius={110} paddingAngle={2} stroke="none">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-white/5">
                {recentRecords.slice(0, 7).map((r, i) => (
